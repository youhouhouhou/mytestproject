...
import com.test.config.service.INodeService;
import com.test.config.service.zkresource.observer.IObserver;
...
public class PropertyGroupManagedBean implements Serializable, IObserver {
    ...
    public void createNode() {
        ...
        boolean created = nodeService.createProperty(ZKPaths.makePath(nodeAuth.getAuthedNode(), newPropertyGroupName),
                null);
        if (created) {
            ...
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage("Property group creation failed.", newPropertyGroupName));
        }
    }
    ...
    private void savePropertyGroup(String fileName, String group, InputStream inputstream) throws IOException {
        ...
        if (!properties.isEmpty()) {
            ...
            if (created) {
                ...
                FacesContext.getCurrentInstance().addMessage(null,
                        new FacesMessage("Failed", fileName + " create group " + group + " failed."));
            }
        } else {
